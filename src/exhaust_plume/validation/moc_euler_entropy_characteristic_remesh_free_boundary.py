...
def measure_moc_euler_ambient_first_wedge_entropy_characteristic_remesh_free_boundary(
  ...
  *,
  ...
  if shock is not None:
    try:
      ...
      for index, (point, expected_state, expected_pressure) in enumerate(path):
        actual_state = remesh.diagnostic_state_at(
          point,
          position_tolerance_m=position_tolerance_m,
        )
        actual_pressure = remesh.diagnostic_static_pressure_at(
          point,
          position_tolerance_m=position_tolerance_m,
        )
  ...
  status_consistent = bool(
    ...
    or (expected_closed and path_coverage_verified and reflected_verified)
    or (
