...
class MocReflectedTraceCompressionProfile:
  ...
  def __post_init__(self) -> None:
    ...
    ####
    for left, right in zip(self.source_trace, self.source_trace[1:]):
      if right.state.y_m >= left.state.y_m:
        raise ValueError(
          'source_trace ordinates must strictly decrease toward the centerline'
        )
      ####
    ####
  ####
  ...
  def _baseline_flow_angle_at(self, ordinate: float) -> float:
    ...
    last = self.source_trace[-1].state.y_m
    if ordinate >= first:
      return float(self.baseline_flow_angles_rad[0])
    ####
    if ordinate <= last:
      return float(self.baseline_flow_angles_rad[-1])
    ####
    for left_index, (left, right) in enumerate(
      zip(self.source_trace, self.source_trace[1:])
    ):
      left_y = left.state.y_m
      right_y = right.state.y_m
      if right_y <= ordinate <= left_y:
        fraction = (left_y - ordinate) / (left_y - right_y)
        return float(
          self.baseline_flow_angles_rad[left_index]
          + fraction * (
            self.baseline_flow_angles_rad[left_index + 1]
            - self.baseline_flow_angles_rad[left_index]
          )
        )
      ####
    ####
    raise ValueError('shock point ordinate lies outside the reflected trace baseline')
  ####
