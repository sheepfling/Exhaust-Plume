...
- a bounded global ambient-axis shoot that treats a caller-supplied upstream
  ...
  exposes no downstream field and cannot promote a chain cell. The strict
  ambient-axis-to-physical-field bridge rechecks that complete appended
  perimeter before assembling a shock/ambient/centerline field, so a scalar
  root with a tangency failure returns a typed boundary failure and no chain
  seed;
...
The planner now also exposes a separate
``solver-owned-quasi-1d-ambient-free-boundary-reference``. It shoots an
effective outlet height from the terminal subsonic total state and an explicit
ambient-pressure target, generates a finite centerline/outlet/free-boundary
perimeter, and attaches a scalar radial field through the exact terminal seam.
Its effective inlet height, downstream length, terminal regularization, and
quasi-one-dimensional area law are explicit assumptions because the terminal
...
thermodynamic/harmonic residuals, and quasi-one-dimensional mass-flow
residual. It retains the embedded mesh's two-dimensional velocity-divergence
value as a visible diagnostic without using it as a gate, so the scalar
reference cannot be mistaken for a full 2-D flow solve.
