...
| `signature-table-mvp-v1` | `signature.table-lookup` -> `plume.signature.spectral-radiant-intensity@1` | Table shape, interpolation, no-extrapolation, fixed-angle exact-only tables, time-axis, partial-result, reproducible fixture digest, measurement-space mismatch guard, explicit LTE line-source and caller-bound LTE population bridge, provenance, and conformance tests | Pending a verified source asset and intrinsic-signature evidence | Versioned table/interpolation and caller-bound LTE line-source/population spectral engineering only; no reactions, non-LTE inference, geometry, ray field, atmosphere, optics, or detector claim |
...
to the explicit LTE line-source and caller-bound population bridge for
source-temperature, composition, and population provenance. Transition
cross-sections and partition functions remain caller-owned; reactions,
non-LTE closure, inferred spectroscopy, and external validation remain out of
scope. This does not change any product lane's claim ceiling or release status.
