...
| --- | --- | --- | --- | --- |
| `planar-moc-primitives-v1` | No provider; standalone `exhaust_plume.models.moc` foundation | 15 scalar round trips, compatibility, an 8-cell open fan, pressure- and turn-prescribed attached/mild-overexpanded compression, reflected free-boundary march, a reusable 44-cell triangular source-boundary strip with domain-bounded state/pressure sampling, a 12-sample constant-`K+` open-strip continuation with 231 nodes/230 cells and a shock probe that advances to sample 10, a 44-cell connected open characteristic zone, a typed shock-path coupling probe with first-missing-sample evidence, a reflected-boundary trace-extension reference, a shock-seeded boundary-conditioned full C+/C- field with pressure lineage, a solver-generated marched attached-shock reference with 9/17/33-sample refinement, a solver-generated five-cell chain reference with state carry and independent 9/17/33-sample chain-refinement measurement, a deterministic three-cell prescribed-boundary chain-planner mock, candidate post-shock total-pressure loss bookkeeping, prescribed-boundary downstream C- traces, a first downstream cross-characteristic layer, independent `op.moc.shock-cell-geometry`, `op.moc.shock-cell-chain`, and `op.moc.shock-cell-chain-refinement` extraction over the reference/planner fixtures, a one-sided caustic restart with exact seed-to-band handoff and typed characteristic-caustic stop, and a separate CJ-UEJ centerline Mach component diagnostic pass their local contracts | CJ-UEJ MOC diagnostic quantifies 10/19 centerline points (RMSE 0.06577) but remains not accepted; coupled reflected-field shock-path closure, physical validity of the constant-`K+` continuation assumption, canonical production free-boundary compression/shock closure, production next-cell shock fitting, disjoint closure evidence, external measurement comparison, and provider mapping remain pending | Numerical planar characteristic diagnostics only; no public VIS/SIG/RAY/FPA claim |
