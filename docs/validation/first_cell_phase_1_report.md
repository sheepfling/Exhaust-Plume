...
remains finite and open. The same bounded field now also reports supplemental
static-pressure-ratio and axial-velocity diagnostics on the centerline and
`y/D=0.25` profile; their partial coverages are recorded individually rather
than extrapolated beyond the open lattice. The centerline/off-axis velocity
comparisons cover 10/19 and 17/45 points (RMSE 18.80 and 12.79 m/s); the
static-pressure comparisons cover 5/69 and 17/31 points (RMSE 0.2144 and
0.0996 in pressure-ratio units). These numbers are not a first-cell or VIS
validation claim: the open mesh has no physical shock closure or complete
post-shock field; the prescribed-boundary C- traces are recorded separately
and remain trace-only, and the source case does not supply a disjoint closure
validation set.
...
`594 passed, 18 warnings`.
Ruff and Pyright both pass. Wall time is environment-dependent and is not
treated as solver benchmark evidence; solver-only timing and peak memory were
not instrumented.
