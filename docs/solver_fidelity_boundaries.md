...
| --- | --- | --- | --- | --- |
| `planar-moc-primitives-v1` | foundation validated; provider pending | Standalone planar characteristic states, scalar inversions, compatibility residuals, pressure- and turn-prescribed attached compression, local ambient-pressure tangent segment, and an open underexpanded fan mesh | None yet; future MOC first-cell provider only after physical closure/topology/convergence gates | No public visual, signature, optical, detector, or FPA claim; no axisymmetric or reacting-flow claim |
...
compression, mild-overexpanded lip-compression branch, local ambient-pressure
tangent residual, and mesh topology checks pass with finite residuals. This is
numerical-foundation evidence, not a
