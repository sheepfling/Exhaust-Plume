...
| --- | --- | --- | --- | --- |
| `planar-moc-primitives-v1` | experimental research visual provider | Standalone planar characteristic states and retained field visualization with scalar inversions, compatibility residuals, pressure- and turn-prescribed attached compression, reflected/free-boundary diagnostics, and independent shock-cell geometry/topology measurement operators | `plume.visual.sectioned-tube@1` as an illustrative research surface only; production promotion requires complete closure, refinement, and external validation | No production visual, signature, optical, detector, or FPA claim; no axisymmetric or reacting-flow claim; the elliptic reference is not a supersonic MOC chain cell |
