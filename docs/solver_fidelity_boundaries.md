...
| --- | --- | --- | --- | --- |
| `planar-moc-primitives-v1` | foundation validated; provider pending | Standalone planar characteristic states, scalar inversions, compatibility residuals, attached compression, local ambient-pressure tangent segment, and an open underexpanded fan mesh | None yet; future MOC first-cell provider only after physical closure/topology/convergence gates | No public visual, signature, optical, detector, or FPA claim; no axisymmetric or reacting-flow claim |
...
8-cell open underexpanded fan, attached-compression inversion, local
ambient-pressure tangent residual, and mesh topology checks pass with finite
residuals. This is numerical-foundation evidence, not a closed first-cell or
product-provider result. The existing basic and reduced-order visual lanes
remain unchanged until a separate MOC assembler passes physical
free-boundary/compression closure, refinement, and measurement-operator gates.
